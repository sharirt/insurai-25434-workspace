...
import { ImportInvestmentTracksFromExcelAction } from "@/product-types";

interface ImportInvestmentTracksDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
  clientId: string;
...
export function ImportInvestmentTracksDialog({
  open,
  onClose,
  onSuccess,
  clientId,
}: ImportInvestmentTracksDialogProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [fileError, setFileError] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const { uploadFunction, isLoading: isUploading } = useFileUpload();
  const { executeFunction, isLoading: isExecuting } = useExecuteAction(
    ImportInvestmentTracksFromExcelAction
  );

  const isLoading = isUploading || isExecuting;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      let message = `יובאו ${importedCount} מסלולי השקעה בהצלחה`;
      if (skippedCount > 0) {
      ...
      if (importedCount > 0) {
        ...
        toast.error("לא יובאו מסלולי השקעה");
      }
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "אירעה שגיאה בייבוא מסלולי ההשקעה. אנא נסה שוב.";
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[440px]" dir="rtl">
        <DialogHeader>
          <DialogTitle>ייבוא מסלולי השקעה מאקסל</DialogTitle>
          <DialogDescription>
            העלה קובץ אקסל לייבוא מסלולי השקעה עבור לקוח זה. מספרי הפוליסה
            ישויכו אוטומטית לקרנות הקיימות של הלקוח.
          </DialogDescription>
        </DialogHeader>
