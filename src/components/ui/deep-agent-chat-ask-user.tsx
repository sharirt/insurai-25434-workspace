...
export type AskUserQuestionOption = NonNullable<AskUserQuestion["options"]>[number];
...
const normalizeAskUserOptions = (question: AskUserQuestion): AskUserQuestionOption[] => {
...
const shouldRenderAsCards = (question: AskUserQuestion, options: AskUserQuestionOption[]) => {
...
const getQuestionOptionLabel = (options: AskUserQuestionOption[], optionId: string) =>
  options.find((option) => option.id === optionId)?.label ?? optionId;

const askUserAnswersToFormState = (submittedAnswers: AskUserSubmittedAnswer[] | undefined) => {
...
const askUserCardDescriptionVariants = cva("mt-0.5 text-muted-foreground leading-snug", {
  variants: {
    size: {
      sm: "text-xs",
      md: "text-sm",
...
const askUserChipsGroupClass = "flex w-full min-w-0 flex-wrap items-center justify-start";
const askUserCardsGroupClass = "flex w-full min-w-0 flex-col items-stretch justify-start";
...
const askUserGroupSpacing = (size: AskUserSize): number => (size === "sm" ? 1.5 : size === "lg" ? 2.5 : 2);
...
function SendArrowButton({ size, label, onClick, disabled }: AskUserActionButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function BackArrowButton({ size, label, onClick, disabled }: AskUserActionButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
  ...
  return (
    <>
      <span className="flex min-w-0 flex-1 flex-col">
        <span className={cn(askUserCardTitleVariants({ size }))}>{option.label}</span>
        {option.description && (
          <span className={cn(askUserCardDescriptionVariants({ size }))}>{option.description}</span>
        )}
      </span>
      {multi && (
        <span className={cn(askUserCardCheckIndicatorVariants({ size, selected }))} aria-hidden="true">
          {selected && <Check className={cn(askUserCardCheckIconVariants({ size }))} />}
        </span>
      )}
    </>
  );
...
export function AskQuestionTool({
  ...
  const initialFormState = React.useMemo(() => askUserAnswersToFormState(answers), [answers]);
  const [formAnswers, setFormAnswers] = React.useState<Record<string, string | string[]>>(
    initialFormState.answerValues,
  );
  const [otherAnswers, setOtherAnswers] = React.useState<Record<string, string>>(initialFormState.otherAnswerValues);
  const [invalidQuestionId, setInvalidQuestionId] = React.useState<string | null>(null);
  ...
  const setSingleValue = React.useCallback((questionId: string, value: string) => {
    setFormAnswers((current) => ({ ...current, [questionId]: value }));
  }, []);

  const setMultiValues = React.useCallback((questionId: string, values: string[]) => {
    ...
    (
      ...
      questions.map((question): AskUserSubmittedAnswer => {
        ...
        if (question.type === "multi_select") {
          ...
          const realValues = values.filter((value) => value !== ASK_USER_OTHER_OPTION_ID);
          return {
            ...
            other: values.includes(ASK_USER_OTHER_OPTION_ID) ? otherAnswer : undefined,
          };
        }
        ...
        if (question.type === "single_select") {
          ...
          return {
            ...
            label: value && value !== ASK_USER_OTHER_OPTION_ID ? getQuestionOptionLabel(options, value) : undefined,
  ...
  const validateAnswers = React.useCallback(
    (sourceAnswers: Record<string, string | string[]>, sourceOtherAnswers: Record<string, string>) => {
      for (const question of questions) {
        ...
        if (question.type === "single_select" && (typeof answer !== "string" || !answer)) {
          return question.id;
        }
        ...
        if (Array.isArray(answer) && answer.includes(ASK_USER_OTHER_OPTION_ID) && !otherAnswer) {
          return question.id;
        }
      }
      return null;
  ...
  const submitWith = React.useCallback(
    async (sourceAnswers: Record<string, string | string[]>, sourceOtherAnswers: Record<string, string>) => {
      if (!respond) {
        return;
      }
      ...
      setSubmitted(true);
      await respond({ answers: buildSubmittedAnswers(sourceAnswers, sourceOtherAnswers) });
    },
    [buildSubmittedAnswers, respond, validateAnswers],
  );

  const firstQuestion = questions[0];
  const firstQuestionOptions = firstQuestion ? normalizeAskUserOptions(firstQuestion) : [];
  ...
  return (
    <FieldGroup
      className={cn(askUserWrapperVariants({ size }), "[container-type:normal]")}
    ...
    >
      {questions.map((question) => {
        ...
        const currentSingleValue = typeof currentAnswer === "string" ? currentAnswer : "";
        ...
        return (
          ...
          >
            <FieldLabel
              ...
              className={cn(askUserQuestionLabelVariants({ size, invalid: isInvalid }))}
            >
              {question.label}
            </FieldLabel>
                ...
                {isMulti ? (
                    ...
                    {options.map((option) => (
                      <ToggleGroupItem
                        ...
                        {...(isCards && option.description
                          ...
                          : {})}
                        className={cn(isCards ? askUserCardItemVariants({ size }) : askUserChipItemVariants({ size }))}
                      >
                        {isCards ? (
                          <CardItemChildren
                            option={option}
                  ...
                  >
                    {options.map((option) => (
                      ...
                      >
                        {isCards ? (
                          <CardItemChildren option={option} size={size} selected={currentSingleValue === option.id} />
                        ) : (
                          <span>{option.label}</span>
                        )}
                      </ToggleGroupItem>
                    ...
                    <BackArrowButton
                      ...
                      onClick={() => {
                        if (isMulti) {
                          setMultiValues(
                            question.id,
                            currentValues.filter((v) => v !== ASK_USER_OTHER_OPTION_ID),
                          );
                        } else {
      ...
      {!isInstantSubmitFlow && !isDisabled && (
        <div className={cn(askUserActionRowVariants({ size }))}>
          <SendArrowButton size={size} label="Send" disabled={isDisabled} onClick={handleSendClick} />
        </div>
      )}
    </FieldGroup>
  );
  ...
  return (
    ...
    >
      {Array.from({ length: safeCount }).map((_, questionIndex) => (
        <Field key={questionIndex} className={cn(askUserQuestionGapVariants({ size }))}>
          <Skeleton className={cn(askUserSkeletonLabelVariants({ size }), "w-3/4")} />
          <div className={cn("flex flex-wrap", askUserSkeletonChipsRowGap[size])}>
            {SKELETON_CHIP_WIDTHS.map((widthClass, chipIndex) => (
              <Skeleton key={chipIndex} className={cn(askUserSkeletonChipVariants({ size }), widthClass)} />
            ))}
          </div>
        </Field>
      ))}
