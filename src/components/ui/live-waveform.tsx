...
export const LiveWaveform = ({
      ...
      if (active && currentTime - lastUpdateRef.current > updateRate) {
        ...
        if (analyserRef.current) {
          ...
          if (mode === 'static') {
            ...
            // Maintain history size — use container width to ensure we always
            // have enough bars to fill the visible area
            const containerWidth = containerRef.current?.getBoundingClientRect().width || 200;
            const maxHistory = Math.max(historySize, Math.ceil(containerWidth / (barWidth + barGap)) + 5);
  ...
  return (
    <div
      ...
      ref={containerRef}
      style={{ height: heightStyle, ...propStyle }}
      aria-label={
        active
          ? 'Live audio waveform'
          : processing
