...
לכל מוצר, החזר את שדה fundId עם הערך של "מזהה קרן" שסופק בנתונים.`;
...
export default function PortfolioRiskAnalysis() {
  ...
  const buildFundBaseDataMap = (): Record<string, FundBaseData> => {
    ...
    for (const f of allClientFunds) {
      ...
      const fundTotal = f.totalBalance ?? fundTracks.reduce((s: number, t: any) => s + (t.trackAccumulationAmount ?? 0), 0);
      const fundKey = f.id ?? f.planName ?? "";
      map[fundKey] = {
      ...
      // Also key by planName for fallback matching
      if (f.planName && f.id && f.planName !== f.id) {
        map[f.planName] = map[fundKey];
      }
    }
    return map;
  };
